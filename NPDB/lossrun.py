...
def update_files_in_path(root='./data/pdfs/', log_file='log_file.txt'):
    ...
    # get the log of the last modified files
...
def transform_to_images_an_entire_folder(pdfs_folder='./data/pdfs/', images_folder='./data/images/', format='.png',
...
def transform_to_text_an_entire_folder(images_folder='./data/images/', text_folder='./data/txt/', save_string=False,
    ...
    try:
        ...
        print("Error in path: " + images_folder, "It doesn't exist or wrong path name")
        return False
...
def spatial_filter(txt_dict, topics, report_type):
    """Creates a list of words related by position related in ,
    Args:
        txt_dict (dict): Dictionary with raw txt info in pdf report
        topics (list): Topics list of the entities of interest
        report_type:
    Returns:
    ...
    for topic in range(len(topics)):
        ...
        if report_type == 'LOSSRUN':
            for i in range(len(txt_dict['text'])):
                ...
                if l - w < txt_left < l + w and txt_top > t:
            ...
            for i in range(len(txt_dict['text'])):
                ...
                if (t - h < txt_top < t + h and txt_left > l) and (txt_left - (l + w) < 300):
            ...
            for i in range(len(txt_dict['text'])):
                ...
                if (t - h < txt_top < t + h and txt_left > l + w) and (txt_left + -(l + w) < 850):
                    horizontal_candidates.append(txt_text)
            ...
            for i in range(len(txt_dict['text'])):
                ...
                if (t - h < txt_top < t + h and txt_left > l) and (txt_left - (l + w) < 800):
                    horizontal_candidates.append(txt_text)
        else:
            ...
            for i in range(len(txt_dict['text'])):
                ...
                if t - h < txt_top < t + h and txt_left > l:
                    horizontal_candidates.append(txt_text)
...
def extract_statistic_features(list_of_paths_of_txt_files):
    ...
    return np.array(
        [mean_x] + [mean_y] + [std_x] + [std_y] + [size_x] + [size_y] + [words_number] + [no_claims] + [max_levels] + [
            level_average])
...
def search_rules(dictionary, rules):
    ...
    # map words positions in text list
        ...
        for i in range(len(rules[item])):
            ...
            if rules[item][i] in sentence:
                ...
                while rules[item][i] in sentence:
                    ...
                    try:
                        _text_temp_dict[poss.index(sentence.index(rules[item][i])) + 1] = '?' * len(
                            _text_temp_dict[poss.index(sentence.index(rules[item][i])) + 1])
                        rules_coords += [(item, rules[item][i], poss.index(sentence.index(rules[item][i])) + 1,
                                          _temp_dict['left'][poss.index(sentence.index(rules[item][i])) + 1],
                                          _temp_dict['top'][poss.index(sentence.index(rules[item][i])) + 1])]
                        sentence = ' '.join(_text_temp_dict)
                    except:
                        break
                ...
                while asociate_terms[0] in sentence:
                    try:
                        ...
                        for j in range(len(_text_temp_dict)):
                            dist_euc = np.sqrt(
                                (coord_x - _temp_dict['left'][j]) ** 2 + (coord_y - _temp_dict['top'][j]) ** 2)
                        ...
                        if all(elem in _aux for elem in asociate_terms):
                            ...
                            _text_temp_dict[poss.index(sentence.index(asociate_terms[0])) + 1] = '}' * len(
                                _text_temp_dict[poss.index(sentence.index(asociate_terms[0])) + 1])
                            rules_coords += [(
                                item, asociate_terms[0], poss.index(sentence.index(asociate_terms[0])) + 1,
                                _temp_dict['left'][poss.index(sentence.index(asociate_terms[0])) + 1],
                                _temp_dict['top'][poss.index(sentence.index(asociate_terms[0])) + 1])]
                            sentence = ' '.join(_text_temp_dict)
...
def is_report(image, txt):
    ...
    if ('FROM:' in content.upper()) and ('SENT:' in content.upper()) and ('@' in content.upper()):
        ...
        return (
            'LOSSRUN', ConfigObj('config/config_lossrun_topics.ino'), ConfigObj('config/config_lossrun_entities.ino'))
    else:
        return 'any'
