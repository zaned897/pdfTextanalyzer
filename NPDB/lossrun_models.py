...
class LossRunFact(Base):
    ...
    @validates('loss_date', 'policy_id', 'loss_reported_date', 'status_id', 'lossrunreport_id', 'reportgenerator_id',
               'insurer_id', 'insured_id')
    ...
    def __init__(self, loss_date, policy_id, loss_reported_date, claim_reference, status_id, claimant_name,
                 expense_reserve, indemnity_reserve, expense_paid, indemnity_paid, total_incurred, relevant,
                 lossrunreport_id,
...
def registerRecord(**kwargs):
    emptyArgs = {'timeDimDay': None, 'timeDimMonth': None, 'timeDimYear': None, 'statusDim': None,
                 'insurerDimName': None, 'insurerDimAddress': None,
                 'insurerDimStatus': None, 'insuredDimName': None, 'insuredDimAddress': None, 'insuredDimStatus': None,
                 'policyDimInsuredId': None,
                 'policyDimInsurerId': None, 'policy_number': None, 'statusName': None, 'policyDimStartDate': None,
                 'policyDimEndDate': None, 'expenseReserve': None, 'claimantName': None, 'policyDimStatus': None,
                 'lossRunReportDimLoadDate': None,
                 'lossRunReportDimDate': None, 'reportGeneratorDimName': None, 'reportGeneratorDimAddress': None,
                 'reportGeneratorDimStatus': None, 'claimReference': None, 'indemnityReserve': None,
                 'expensePaid': None, 'indemnityPaid': None,
...
def register(data):
    ...
    insurerDim = validateData(InsurerDim, 'insurer_id', data['insurerDimName'], data['insurerDimAddress'],
                              data['insurerDimStatus'])
    insuredDim = validateData(InsuredDim, 'insured_id', data['insuredDimName'], data['insuredDimAddress'],
                              data['insuredDimStatus'])
    policyDim = validateData(PolicyDim, 'policy_id', data['policyDimInsuredId'], data['policyDimInsurerId'],
                             data['policy_number'], data['policyDimStartDate'], data['policyDimEndDate'],
                             data['policyDimStatus'])
    lossRunReportDim = validateData(LossRunReportDim, 'lossrunreport_id', data['lossRunReportDimLoadDate'],
                                    data['lossRunReportDimDate'])
    reportGeneratorDim = validateData(ReportGeneratorDim, 'reportgenerator_id', data['reportGeneratorDimName'],
                                      data['reportGeneratorDimAddress'], data['reportGeneratorDimStatus'])
    fact = LossRunFact(timeDim, policyDim, timeDim, data['claimReference'], statusDim, data['claimantName'],
                       data['expenseReserve'], data['indemnityReserve'], data['expensePaid'], data['indemnityPaid'],
                       data['totalIncurred'], data['relevant'], lossRunReportDim, reportGeneratorDim, insurerDim,
                       insuredDim)
    Session.add(fact)
    Session.commit()
...
def npdbRecord(**kwargs):
    emptyArgs = {'event_day': None, 'event_outcome': None, 'event_paid_by': None, 'payment_date': None,
                 'payment_total_amount': None, 'action_initial': None,
...
npdbRecord(process_date=datetime.datetime.now(), practitioner_name="Test practitioner",
           action_initial='Test initial action',
           action_basis='Basis', entity_name='Entity first name', payment_date=datetime.datetime.now(),
           payment_total_amount=5000, event_day=datetime.datetime.now(),
